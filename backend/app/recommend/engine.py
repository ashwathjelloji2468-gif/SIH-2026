    ...
    def _select_candidates(
        ...
        if purpose == CryptoPurpose.KEY_ESTABLISHMENT or any(k in alg_upper for k in ["ECDH", "X25519", "X448", "DH", "DIFFIE"]) or (any(k in alg_upper for k in ["RSA"]) and purpose == CryptoPurpose.KEY_ESTABLISHMENT):
            tradeoffs = {
                ...
                "performance_notes": "Fast encapsulation and decapsulation efficiency. Minimal network payload impact.",
                "latency_impact": "Low CPU encapsulation latency. Public key size: 800 bytes (ML-KEM-768), ciphertext: 1088 bytes. Minimal network RTT impact.",
                "cost_impact": "Low to Moderate implementation cost. Software dependency update required (OpenSSL 3.2+ / liboqs / BouncyCastle).",
                "latency_level": "LOW",
                "cost_level": "MODERATE",
        ...
        if purpose in [CryptoPurpose.DIGITAL_SIGNATURE, CryptoPurpose.SIGNATURE, CryptoPurpose.AUTHENTICATION] or any(k in alg_upper for k in ["ECDSA", "ED25519", "ED448", "DSA", "RSA"]):
            tradeoffs = {
                ...
                "performance_notes": "High verification throughput and fast CPU signing.",
                ...
                "cost_impact": "Moderate to High cost. Requires PKI certificate chain re-issuance, trust store updates, and application buffer expansion.",
